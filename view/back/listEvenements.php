...
/* ── PARTICIPATION PILL ── */
.part-pill{display:inline-flex;align-items:center;gap:5px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;text-decoration:none;transition:all .15s;border:1.5px solid transparent}
.part-pill:hover{transform:translateY(-1px);box-shadow:0 2px 8px rgba(0,0,0,0.1)}
.part-pill.free{background:#dcfce7;color:#15803d;border-color:#86efac}
.part-pill.free:hover{background:#bbf7d0}
.part-pill.almost{background:#fff7ed;color:#b45309;border-color:#fed7aa}
.part-pill.almost:hover{background:#ffedd5}
.part-pill.full{background:#fce8e8;color:#b91c1c;border-color:#f09595}
.part-pill.full:hover{background:#fecaca}
      ...
      <table id="eventsTable">
        <thead>
          <tr>
            ...
            <th class="sortable" data-col="8" onclick="sortTable(8)">Prix <span class="sort-arrow"></span></th>
            <th>Statut</th>
            <th>Actions</th>
          </tr>
        </thead>
...
function exportCSV() {
  const headers = ['#','Titre','Type','Lieu','Début','Fin','Capacité','Participations','Prix','Statut'];
  const q      = document.getElementById('searchInput').value.toLowerCase();
  const type   = document.getElementById('filterType').value.toLowerCase();
  const statut = document.getElementById('filterStatut').value.toLowerCase();
